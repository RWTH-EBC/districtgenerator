...
class House(Device):
    ...
    def initializeParameters(self, site):
        ...
        self.ecs_storage = ("BAT", "TES")  # battery (BAT), thermal energy storage (TES), electric vehicle (EV)
    ...
    # Storage discharging [W]
        ...
        self.res_load = self.m.addVars(
            self.timesteps,
            vtype=gp.GRB.CONTINUOUS, lb = 0.0, name="res_load_" + str(self.id)
        )

        # Residual electricity injection [W]
        self.res_inj = self.m.addVars(
            self.timesteps,
            vtype=gp.GRB.CONTINUOUS, lb = 0.0, name="res_inj_" + str(self.id)
        )

        # Residual gas demand [W]
        self.res_gas = self.m.addVars(
            self.timesteps,
            vtype=gp.GRB.CONTINUOUS, lb = 0.0, name="res_gas_" + str(self.id)
        )
    ...
    def setConstraints(self):
        ...
        # For buildings with a supply temperature of 35 degrees Celsius
        if self.data["envelope"].construction_year >= 1995 and self.data["capacities"]["HP"] > 0.0:
            # Heat production of EH concerning domestic hot water (DHW) temperature
            ...
            self.m.addConstrs(
                (self.heat["HP", t] ==
                (self.power["HP", t] * self.decentralDevs["HP"]["grade"] * (273.15 + 40) / (40 - (self.T_e[t] - 5)))
                for t in self.timesteps),
                name="Conversion_HP_" + str(self.id))

        # For buildings with a supply temperature of 55 degrees Celsius
        elif self.data["envelope"].construction_year < 1995 and self.data["capacities"]["HP"] > 0.0:
            self.m.addConstrs(
            ...
            self.m.addConstrs(
                (self.heat["HP", t] ==
                (self.power["HP", t] * self.decentralDevs["HP"]["grade"] * (273.15 + 60) / (60 - (self.T_e[t] - 5)))
                for t in self.timesteps),
                name="Conversion_HP_" + str(self.id))

        if self.data["capacities"]["HP"] == 0.0:
            self.m.addConstrs(
                    (self.power["EH", t] <= 0
                    for t in self.timesteps),
                    name="DHW_EH_minimum_" + str(self.id))
        ...
        self.m.addConstrs(
            (self.power["EH", t] == self.heat["EH", t] * self.decentralDevs["EH"]["eta_th"]
             for t in self.timesteps),
        ...
        # TODO: at home rausschmeißen
        #occ = {}
        #for t in self.timesteps:
        #    occ[t] = min(1, self.occupants[t])

        #self.m.addConstrs(
        #    (self.ch["EV", t] == 0 #self.decentralDevs["EV"]["ch_max"] #* occ[t]
        #     for t in self.timesteps),
        ...
        for t in self.timesteps:
            if t == 0:
                # Initial SoC
                soc_prev = self.decentralDevs["TES"]["init"] * self.data["capacities"]["TES"]
            else:
                soc_prev =  self.soc["TES", t - 1]
            self.m.addConstr(
                self.soc["TES", t] == soc_prev * self.decentralDevs["TES"]["eta_standby"]
                + self.dt * (self.ch["TES", t] * self.decentralDevs["TES"]["eta_ch"]
                             - self.dch["TES", t] / self.decentralDevs["TES"]["eta_ch"]),

            name="Storage_balance_TES_" + str(self.id))

        # Heat balances
        self.m.addConstrs(
            (self.dch["TES", t] == self.Q_heating[t] + self.Q_DHW[t]
             for t in self.timesteps),
            name="TES_discharging_" + str(self.id))

        self.m.addConstrs(
            (sum(self.heat[dev, t] for dev in self.ecs_heat) + self.heat_fromGrid[t] == self.ch["TES", t]
            for t in self.timesteps),
            name="TES_charging_" + str(self.id))

        # Energy balance BAT
        for t in self.timesteps:
            if t == 0:
                # Initial SoC
                soc_prev = self.decentralDevs["BAT"]["init"] * self.data["capacities"]["BAT"]
            else:
                soc_prev = self.soc["BAT", t - 1]
            self.m.addConstr(
                self.soc["BAT", t] == soc_prev * self.decentralDevs["BAT"]["eta_standby"]
                + self.dt * (self.ch["BAT", t] * self.decentralDevs["BAT"]["eta_ch"]
                             - self.dch["BAT", t] / self.decentralDevs["BAT"]["eta_ch"]),

                name="Storage_balance_BAT_" + str(self.id))


        # Electricity balance
        self.m.addConstrs(
            (self.res_load[t] + self.power["PV", t] + self.power["FC", t] + self.power["CHP", t] + self.dch["BAT", t]
             == self.res_inj[t] + self.elec_demand[t] + self.ch["BAT", t] + self.power["HP", t] + self.power["EH", t]  # + self.ch["EV", t]
             for t in self.timesteps),
        ...
        self.m.addConstrs(
            (self.res_inj[t] >= 0
             for t in self.timesteps),
            name="Injection_limit2_" + str(self.id))
