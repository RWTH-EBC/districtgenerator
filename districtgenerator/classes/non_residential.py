...
class NonResidential(object):
    ...
    These are:
        ...
        - prj.buildings[self.id].window_area -> has to be calculated, as it is not present in data NWG.
        ...
        if self.facade_estimation_factors["ow1"] != 0:
            for key, value in self._outer_wall_names.items():
                ...
                for zone in self.thermal_zones:
                    ...
                    self.outer_area[key]["area"] = (self.facade_estimation_factors["ow1"] * zone.area) / len(self._outer_wall_names)
        # Windows
        if self.facade_estimation_factors["win1"] != 0:
            for key, value in self._window_names.items():
               ...
               for zone in self.thermal_zones:
                    ...
                    self.window_area[key]["area"] = (self.facade_estimation_factors["win1"] * zone.area) / len(self._window_names)
        # Ground floor
        if self.facade_estimation_factors["gf1"] != 0:
            for key, value in self._ground_floor_names.items():
                ...
                for zone in self.thermal_zones:
                    self.outer_area[key]["tilt"] = value[0]
                    self.outer_area[key]["orientation"] = value[1]
                    self.outer_area[key]["area"] = (self.facade_estimation_factors["gf1"] * zone.area) / len(self._ground_floor_names)
        # Rooftop
        if self.facade_estimation_factors["rt1"] != 0:
            for key, value in self._roof_names.items():
                ...
                for zone in self.thermal_zones:
                    self.outer_area[key]["tilt"] = value[0]
                    self.outer_area[key]["orientation"] = value[1]
                    self.outer_area[key]["area"] = (self.facade_estimation_factors["rt1"] * zone.area) / len(self._roof_names)
