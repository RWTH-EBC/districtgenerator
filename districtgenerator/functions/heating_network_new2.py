...
def network_optimization3(data):
    """
...
def load_parameter(data):
    ...
    k_pipe = heat_grid_data[
        "k_PUF"]  # 0.03 W/(m*K),    Thermal conductivity of pipe insulation materials, corresponding to λ_i in EN 13941.
    ...
    path = {"EH1": extract_longest_branches(network), "EH2": extract_longest_branches(network_wh, "EH2")}
...
def calc_flow(data, param, heat_loss_pipe=None, heat_loss_pipe_cluster=None, save_path=None):
    ...
    network = data.pipeline_topology
    pipe_loads_eh1 = aggregate_heat_loads(network, building_demand, heat_loss_pipe, root="EH1")
    max_pipe_flow_eh1 = {}
    for (parent, child) in pipe_loads_eh1:
        pipe_id = f"{parent}->{child}"
        loads_array = pipe_loads_eh1[(parent, child)]
        pipe_flow_eh1 = loads_array * 1000 / (c_f * deltaT * rho_f)  # m³/s
        max_pipe_flow_eh1[pipe_id] = float(np.max(pipe_flow_eh1))
    ...
    M = 100000  # big M for linearization of product of binary and continuous variable
    ...
    # load on a pipe comint from a source s for each timestep
    ...
    # load on each pipe for each timestep
    ...
    # maximal load on each pipe according to the N-2 criteria
    ...
    model.heat_loss_pipe_source = pyo.Var(model.sources, model.pipe, model.week, model.t, within=pyo.NonNegativeReals,
    ...
    ## binary variables
    # 0 if waste heat source meets the entire heat demand, else 1
    ...
    # %% INTERMEDIATE STEP: DEFINE CONSTRAINTS FOR OPTIMAL HEAT SUPPLY STRATEGY
    ...
    def demand_rule(model, building, week, t):
        ...
        return building_demand_cluster[building][i, t] == sum(
            (model.load_source_sink[building, source, week, t] - model.heat_loss_source_sink[building, source, week, t]) for source in model.sources)
    ...
    # 4) assigns the heat loss to each pipe
    ...
    # 5) constraints to distribute heat_loss to each source sink combination
    def heat_loss_allocation_rule(model, pipe, week, t):  # constraint to evenly distribute the heat loss to each load_source_sink which flows through the given pipe
        return (sum(model.heat_loss_allocation[building, source, pipe, week, t] for building in model.buildings for source in
                    model.sources if normalized(key_to_tuple(pipe)) in sorted_edges[(source, building)])
                == model.heat_loss_pipe[pipe, week, t])

    def heat_loss_source_sink_rule(model, building, source, week, t):  # For each source–sink pair and timestep, the total heat loss is defined  as the sum of the allocated pipe-specific heat losses along the  corresponding network path between source and building.
    ...
    model.heat_loss_allocation_rule = pyo.Constraint(model.pipe, model.week, model.t, rule=heat_loss_allocation_rule)

    # 6) assignment of flow for each source on each pipe
    def pipe_load_spec_rule(model, source, pipe, week, t):
        ...
        for building in model.buildings:
            ...
            if normalized(pipe_tuple) in {normalized(e) for e in path}:  # checks if the pipe is traversed
                ...
                load_sum += model.load_source_sink[building, source, week, t]

        return model.pipe_load_source[source, pipe, week, t] == (load_sum - load_reduction)

    model.pipe_load_spec_rule = pyo.Constraint(model.sources, model.pipe, model.week, model.t, rule=pipe_load_spec_rule)

    # 7) adds the pipe flows coming from each source
    ...
    # 9) Heating networks are typically designed according to the N-2 criterion,
    #  meaning pipes must be sized so that the energy hub can supply the entire demand alone.
    def n_2_rule1(model, pipe, week, t):
        i = week_to_i[week]
        return model.max_pipe_flow_robust[pipe] >= model.pipe_load[pipe, week, t] * 1000 / (c_f * deltaT_cluster[i, t] * rho_f)

    def n_2_rule2(model, pipe):
        return model.max_pipe_flow_robust[pipe] >= max_pipe_flow_eh1[pipe]

    model.n_2_rule1 = pyo.Constraint(model.pipe, model.week, model.t, rule=n_2_rule1)
    model.n_2_rule2 = pyo.Constraint(model.pipe, rule=n_2_rule2)
    ...
    model.objective = pyo.Objective(expr=sum(model.max_pipe_flow_robust[pipe] for pipe in model.pipe), sense=pyo.minimize,
...
def optimization_diameter(data, param):
    ...
    # 5 pipe parameters
    dp_pipe_max = heat_grid_data["pipe"][
        "dp_pipe_max"]  # 300Pa/m,      maximum pipe pressure gradient (Planungshandbuch Fernwärme)
    dp_pipe_min = heat_grid_data["pipe"][
        "dp_pipe_min"]  # 30Pa/m,       minimum pipe pressure gradient (Improved genetic algorithm for pipe diameter optimization of an existing large-scale district heating network https://doi.org/10.1016/j.energy.2024.131970)
    ...
    # 6 economic factor
    # 1) pipe
    cost_om_pipe = heat_grid_data["pipe"][
        "cost_om_pipe"]  # 0.005         pipe operation and maintenance costs as share of investment (VDI 2067)
    ...
    price_el_pumps = data.ecoData["price_supply_el_eh"][
        0]  # 0.3141€/kWh,  electricity costs for pump supply used for network design (equals LEC of CHP for 7000 full load hours)
    ...
    cost_om_HP = data.central_device_data["AirHP"][
        "cost_om"]  # 0.025,        1/year (fraction of inv_var), source: VDI2067
    ...
    devs_param = {
        "feasible": True,
        "dT_evap": 10,
        # K,    temperature difference in evaporator (how much the air cools down in the evaporator); Source: JENSEN J. et al. Heat pump COP, part 2: generalized COP estimation of heat pump processes
        "dT_cond": deltaT_cluster,
        # K,    temperature difference in condenser (how much network's fluid heats up in the condenser)
        "dT_pinch_cond": 2,
        # K,    temperature difference between both fluids in the condenser at pinch point; Source: Klingebiel et al. https://doi.org/10.1016/j.enbuild.2023.113397
        "dT_pinch_evap": 5,  # K,    temperature difference between both fluids in the evaporator at pinch point
        "eta_compr": 0.8,
        # ---,  isentropic efficiency of compression; Source: Wirtz et al. https://doi.org/10.1016/j.apenergy.2019.114158
        "heatloss_compr": 0.3,
        # ---,  heat loss rate of compression; # Source: JENSEN J. et al. Heat pump COP, part 2: generalized COP estimation of heat pump processes
    ...
    p_co2 = data.params_ehdo_model[
        "co2_tax"]  # 0,            carbon pricing (0.055€/kg in Germany in 2025 from website https://carbonpricingdashboard.worldbank.org/compliance/price)
    ...
    heat_loss_prefac = (
                                   price_el_pumps + p_co2 * EF) / COP_HP  # €/kWh,  total unit cost of producing heat to cover network heat losses.
    ...
    print(f"Hier sind die Kandidaten: {pipe_dict}")
    s = np.max(list(pipe_dict.keys()))
    print(f"Hier ist der maximalewert: {np.max(list(pipe_dict.keys()))}")
    print(pipe_dict[s]["symmetrical heat loss factor"])
    ...
    # minimal allowed pipe diameter
    model.d_min_t = pyo.Var(model.pipe, model.week, model.t, within=pyo.NonNegativeReals, doc="minimal allowed pipe diameter")
    # maximal allowed pipe diameter
    model.d_max_t = pyo.Var(model.pipe, model.week, model.t, within=pyo.NonNegativeReals, doc="maximal allowed pipe diameter")
    ...
    def assignment_rule(model, building, source, week, t):
        i = week_to_i[week]
        return building_demand_cluster[building][i, t] * model.assignment[building, source, week, t] == model.load_source_sink[building, source, week, t]

    model.assignment_rule = pyo.Constraint(model.buildings, model.sources, model.week, model.t, rule = assignment_rule)
    ...
    model.wh_cap_rule = pyo.Constraint(model.week, model.t, rule=wh_cap_rule)
     ...
     # 3) Generated heat by the energy hub equals the difference between the heat demand and the generated heat by the waste heat source (ensures that waste heat source gets prioritized)
    def eh_cap_rule1(model, week, t):
        return model.heat["EH2", week, t] >= sum(model.load_source_sink[building, source, week, t] for building in model.buildings for source in model.sources) - M * model.q[week, t]

    def eh_cap_rule2(model, week, t):
        return model.heat["EH2", week, t] <= sum(model.load_source_sink[building, source, week, t] for building in model.buildings for source in model.sources) + M * (1-model.q[week, t])

    def eh_cap_rule3(model, week, t):
        return model.heat["EH1", week, t] <= M*model.q[week, t]

    def eh_cap_rule4(model, week, t):
        return model.heat["EH1", week, t] <= sum(model.load_source_sink[building, source, week, t] for building in model.buildings for source in model.sources) - model.heat["EH2", week, t] + M*(1-model.q[week, t])
...
#    model.heat_loss_pipe_com3_rule = pyo.Constraint(model.buildings, model.sources, model.week, model.t, rule=heat_loss_pipe_com_rule3)
    ...
    def pipe_flow_spec_rule(model, source, pipe, week, t):
                    ...
                    if normalized(edge) == normalized(pipe_tuple):
                        ...
                        if edge_str1 in model.pipe:
                            if edge[1] in model.buildings:
                                load_reduction += (model.assignment[edge[1], source, week, t] * building_demand_cluster[edge[1]][i, t])
                            else:
                                load_reduction += 0
        ...
        return model.pipe_flow_source[source, pipe, week, t] == load_sum * 1000 / (c_f * deltaT_cluster[i, t] * rho_f)

    model.pipe_flow_spec_rule = pyo.Constraint(model.sources, model.pipe, model.week, model.t, rule=pipe_flow_spec_rule)

    def pipe_flow_rule(model, pipe, week, t):
        return model.pipe_flow[pipe, week, t] == sum(model.pipe_flow_source[source, pipe, week, t] for source in model.sources)
    ...
    def pump_pipe_d_rule(model, source, pipe, d, week, t):
        return model.pump_pipe_d_cand[source, pipe, d, week, t] - model.pump_pipe_d[source, pipe, d, week, t] <= M * (1-model.z[pipe, d])

    model.pump_pipe_d_rule = pyo.Constraint(model.sources, model.pipe_diam, model.week, model.t, rule=pump_pipe_d_rule)

    def pump_pipe_rule(model, source, pipe, week, t):
        return model.pump_pipe[source, pipe, week, t] == sum(model.pump_pipe_d[source, pipe, d, week, t] for d in pipe_candidates[pipe])
    ...
    def p_station_rule(model, source, week, t):
        return model.p_station[source, week, t] == sum(model.flow_source_sink[building, source, week, t] for building in model.buildings) * dp_station_total / (eta_pump * 1000)
    ...
    def pump_el_ge_path_rule(model, source, line, week, t):
        ...
        expr = sum(model.pump_pipe[source, pair_to_pid[(nodes[i], nodes[i + 1])], week, t] for i in range(len(nodes) - 1))
    ...
    def pump_cap_rule(model, week, t):
        return sum(model.pump_el[source, week, t] for source in model.sources) <= model.pump_cap
    ...
    def pump_energy_total_rule(model):
        return model.pump_energy_total == sum(
            sum(model.pump_el[source, w, t] for source in model.sources for t in model.t) * data.clusterWeights[w] for w in model.week)
    ...
    def heat_loss_rule(model, pipe, week, t):
        ks = pyo.quicksum(
            pipe_dict[d]["symmetrical heat loss factor"] * model.z[pipe, d] for d in pipe_candidates[pipe])
        i = week_to_i[week]
        length = data.pipeline[pipe]["length"]
        return model.heat_loss_pipe[pipe, week, t] == 2 * (
                    T_s_cluster[i, t] - T_soil_cluster[i, t]) * 2 * np.pi * k_soil * ks * length / 1000
    ...
    for pipe in data.pipeline.keys():
        ...
        # get the optimized flow of each pipe
        flow = np.zeros((len(weeks), time_steps))
        for i, week in enumerate(weeks):
            for t in range(time_steps):
                flow[i, t] = pyo.value(model.pipe_flow[pipe, week, t])
        data.pipeline[pipe]["flow"] = flow

        flow_source_max = {}
        for source in SOURCES:
            flow_source_max[source] = max(pyo.value(model.pipe_flow_source[source, pipe, week, t]) for week in data.clusters for t in range(time_steps))
            data.pipeline[pipe][f"flow_source_max_{source}"] = flow_source_max[source]


        # get the optimized maximal flow of each pipe segment
        data.pipeline[pipe]["flow_max"] = max(pyo.value(model.pipe_flow[pipe, week, t]) for week in data.clusters for t in range(time_steps))
        print(f"Hier ist der maximale Fluss: {data.pipeline[pipe]['flow_max']}")
    ...
    for pipe in data.pipeline.keys():
        ...
        for week in data.clusters:
            for t in range(time_steps):
                pipe_flows.append(pyo.value(model.pipe_flow[pipe, week, t]))
        flow_max = np.max(pipe_flows)
        d_min_list = []
        d_max_list = []
        for week in data.clusters:
            for t in range(time_steps):
                d_min_list.append(pyo.value(model.d_min_t[pipe, week, t]))
                d_max_list.append(pyo.value(model.d_max_t[pipe, week, t]))
        d_min = np.max(d_min_list)
        d_max = np.max(d_max_list)
        d_min_real = C_min * flow_max**0.4
        d_max_real = C_max * flow_max**0.4

        #d = data.pipeline[pipe]["DN"]
        D_inner = pipe_dict[25]["Inner diameter (pipe) (mm)"] / 1000.0
        K = prefac_pipe[pipe] * length * 2.0 * (1.0 + 0.2) * rho_f / (D_inner ** 5)
        max_err_list = []
        for source in SOURCES:
            for week in data.clusters:
                for t in range(time_steps):
                    flow_source = pyo.value(model.pipe_flow_source[source, pipe, week, t])
                    #for d in pipe_candidates[pipe]:
                     #   if pyo.value(model.z[pipe, d]) > 0.5:
                    pump_power = pyo.value(model.pump_pipe_d_cand[source, pipe, 25, week, t])
                    #pump_power = pyo.value(model.pump_pipe[source, pipe, week, t])
                    pump_power_real = K * ((flow_source * 1000) ** 3)
                    print(f"Hier ist die berechnete: {pump_power} und hier die reale: {pump_power_real}")
                    if pump_power_real > 0:
                        max_err_list.append(abs(pump_power - pump_power_real)/pump_power_real * 100)
        max_err = np.max(max_err_list)
        print(f"Hier ist der maximale Pumpfehler für jeden Pipe: {max_err}%")



        err_dmin_rel = abs(d_min - d_min_real) / d_min_real * 100
        print(f"Hier der Fehler für den minimalen Durchmesser: {err_dmin_rel}%")
        err_dmax_rel = abs(d_max - d_max_real) / d_max_real * 100
        print(f"Hier der Fehler für den maximalen Durchmesser: {err_dmax_rel}%")
        print(f"Hier der minimale reale Durchmesser: {d_min_real} und hier der minimale approximierte Durchmesser: {d_min}")
        print(f"Hier der maximale reale Durchmesser: {d_max_real} und hier der maximale apporximierte Durchmesser: {d_max}")
...
def output_diameter(data, param):
    ...
    print(f"Hier ist die Pumpenkapazität: {pump_cap}")
    ...
    print(f"Hier ist die gesamte Menge in kWh: {pump_energy_total}")
    ...
    print(f"Und hier sind dann die Kosten: {pump_electricity_costs}")
    ...
    print(f"Hier sind die ann costs: {pipes_ann_costs}")
    ...
    results = {
        ...
        "network_total_costs": {
            "value": float(
                network_ann_costs + network_om_costs + pump_electricity_costs + HP_ann_costs + HP_om_costs + HP_electricity_costs),
            "unit": "€",
...
def aggregate_heat_loads_wh(data, network, heat_loss_pipe):
    if heat_loss_pipe is None:
        heat_loss_pipe = {}
    ...
    # number of buildings
    buildings = building_demand_cluster.keys()
    ...
    # returns all edges that connect the start node to the end node
    def get_path_edges(edges, start_node, end_node):
    ...
    # load which is lost by the time it arrives at the given pipe
    def lost_load(pipe, source, building):
        path_edges = get_path_edges(edges, source, building)
        loss = 0
        for edge in path_edges:
            if edge == pipe:
                break
            else:
                loss += heat_loss_pipe.get((edge), 0)

        return loss

    # create model
    model = pyo.ConcreteModel(name="Pipe load optimization")

    # sets
    model.pipes = pyo.Set(initialize=edges, doc="pipes in the network")
    model.sources = pyo.Set(initialize=SOURCES, doc="sources in the network")
    model.buildings = pyo.Set(initialize=buildings, doc="buildings in the network")
    model.weeks = pyo.Set(initialize=weeks, doc="Typical weeks")
    model.t = pyo.Set(initialize=range(time_steps), doc="Time steps within a typical week")

    # decision variables

    # load on edges
    model.pipe_loads = pyo.Var(model.pipes, model.weeks, model.t, within=pyo.NonNegativeReals)

    # flow from source to sink
    model.flow = pyo.Var(model.buildings, model.sources, model.weeks, model.t, within=pyo.NonNegativeReals)

    # produced heat by heat sources
    model.heat = pyo.Var(model.sources, model.weeks, model.t, within=pyo.NonNegativeReals)

    # binary variable to ensure that the heat flows only in one direction within a pipe
    model.y = pyo.Var(model.pipes, model.weeks, model.t, within=pyo.Binary)

    # pipe loss
    model.pipe_loss = pyo.Var(model.pipes, model.weeks, model.t, within=pyo.NonNegativeReals)

    # loss between source and sink
    model.total_loss = pyo.Var(model.buildings, model.sources, model.weeks, model.t, within=pyo.NonNegativeReals)

    # objective function
    # minimize the pipe loads
    def objective_rule(model):
        return sum(model.pipe_loads[pipe, week, t] for pipe in model.pipes for week in model.weeks for t in model.t)

    model.objective = pyo.Objective(rule=objective_rule, sense=pyo.minimize,
                                    doc="Minimize pipe loads")

    # The demand rule ensures that the building demand is covered during all periods. The losses are taken into account.
    def demand_rule(model, building, week, t):
        i = week_to_i[week]
        return building_demand_cluster[building][i, t] == sum(
            model.flow[building, source, week, t] - model.total_loss[building, source, week, t] for source in
            model.sources)

    model.demand_rule = pyo.Constraint(model.buildings, model.weeks, model.t, rule=demand_rule)

    # The heat of the waste heat source is assigned to the generated profile.
    def wh_cap_rule(model, week, t):
        i = week_to_i[week]
        return model.heat["EH2", week, t] == wh_capacity[i, t]

    model.wh_cap_rule = pyo.Constraint(model.weeks, model.t, rule=wh_cap_rule)

    # TODO GLEICHUNG ÄNDERN, UM DIE WÄRME DER ABWÄRMEQUELLE ZU PRIORISIEREN!
    def eh_cap_rule(model, week, t):
        return model.heat["EH1", week, t] >= sum(
            model.flow[building, source, week, t] for building in model.buildings for source in model.sources) - \
            model.heat["EH2", week, t]

    model.eh_cap_rule = pyo.Constraint(model.weeks, model.t, rule=eh_cap_rule)

    def capacity_rule(model, source, week, t):
        return model.heat[source, week, t] >= sum(model.flow[building, source, week, t] for building in model.buildings)

    model.capacity_rule = pyo.Constraint(model.sources, model.weeks, model.t, rule=capacity_rule)

    # heat loss contraints
    def pipe_loss_rule(model, parent, child, week, t):
        pipe = (parent, child)
        loss = heat_loss_pipe.get((pipe), 0)
        return model.pipe_loss[pipe, week, t] == loss

    model.pipe_loss_rule = pyo.Constraint(model.pipes, model.weeks, model.t, rule=pipe_loss_rule)

    def total_loss_rule(model, building, source, week, t):
        loss = 0
        path_edges = get_path_edges(edges, source, building)
        for pipe in model.pipes:

            if normalized(pipe) in {normalized(e) for e in path_edges}:
                loss += model.pipe_loss[pipe, week, t]

        return model.total_loss[building, source, week, t] == loss

    model.total_loss_rule = pyo.Constraint(model.buildings, model.sources, model.weeks, model.t, rule=total_loss_rule)

    # every pipe is assigned with the sum of all loads which cross it during a given time period
    def loads_rule(model, parent, child, week, t):
        pipe = (parent, child)
        flow_sum = 0
        ...
        for building in model.buildings:
            for source in model.sources:
                ...
                load_reduction = lost_load(pipe, source,
                                           building)  # load reduction is the fraction of the load which is already lost by the time it arrives at the given pipe
                path_edges = get_path_edges(edges, source, building)

                if normalized(pipe) in {normalized(e) for e in path_edges}:  # checks if the pipe is traversed
                    flow_sum += (model.flow[building, source, week, t] - load_reduction)

        return model.pipe_loads[pipe, week, t] == flow_sum

    model.loads_rule = pyo.Constraint(model.pipes, model.weeks, model.t, rule=loads_rule)

    # Constraints to ensure that during any given time step, the heat is only allowed to flow in one direction in a pipe
    # if y is 0, the heat is flowing in reverse direction, if y is 1, the heat is flowing in forward direction. y is assigned to every pipe
    def flow_forward_rule(model, parent, child, week, t):
        pipe = (parent, child)
        flow_reverse = 0
        ...
        for source in model.sources:
            for building in model.buildings:
                ...
                for edge in path_edges:
                    if normalized(edge) == normalized(
                            pipe) and edge != pipe:  # if the considered pipe is traversed on the way from the source to the sink but the
                        flow_reverse += model.flow[
                            building, source, week, t]  # heat flows in the opposed direction of the defined flow direction, model.flow is
                        # added to the variable flow_reverse
        return flow_reverse <= M * (
                    1 - model.y[pipe, week, t])  # if y is 0, the flow is allowed in the reverse direction

    def flow_reverse_rule(model, parent, child, week, t):
        pipe = (parent, child)
        flow_forward = 0
        ...
        for source in model.sources:
            for building in model.buildings:
                ...
                for edge in path_edges:
                    if normalized(edge) == normalized(pipe) and edge == pipe:
                        flow_forward += model.flow[building, source, week, t]

        return flow_forward <= M * model.y[
            pipe, week, t]  # if y is 1, the flow is allowed to flow in the forward direction

    model.flow_forward_rule = pyo.Constraint(model.pipes, model.weeks, model.t, rule=flow_forward_rule)
    model.flow_reverse_rule = pyo.Constraint(model.pipes, model.weeks, model.t, rule=flow_reverse_rule)
    ...
    solver.solve(model, tee=True, options=solver_options)

    # Extract results
    pipe_loads_dict = {}
    for pipe in model.pipes:
        weeks_list = list(model.weeks)
        t_list = list(model.t)

        array_2d = np.zeros((len(weeks_list), len(t_list)))
        for i, week in enumerate(weeks_list):
            for j, t in enumerate(t_list):
                array_2d[i, j] = pyo.value(model.pipe_loads[pipe, week, t])

        pipe_loads_dict[pipe] = array_2d

    return pipe_loads_dict
...
def calc_COP(devs_param, temperatures):
    ...
    dt_r_H = 0.2 * (t_h_in + dt_h - (
            t_c_in - dt_c) + (
                                dt_pp_cond + dt_pp_evap)) + 0.2 * dt_h + 0.016  # mean entropic heat difference in condenser deducting dt_pp and assuming an ammonia heat pump
    w_is = 0.0014 * (t_h_in + dt_h - (
            t_c_in - dt_c) + (
                                 dt_pp_cond + dt_pp_evap)) - 0.0015 * dt_h + 0.039  # ratio of isentropic expansion work to isentropic compression work and assuming an ammonia heat pump
...
def calc_f_fric_per_pipe(data, param):
    ...
    for pid, pipe in data.pipeline.items():
        ...
        if flow_max > 0:
            ...
            f_i = f_default  # No flow: friction factor remains unchanged (boundary condition)
