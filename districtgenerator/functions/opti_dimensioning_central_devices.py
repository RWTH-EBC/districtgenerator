...
def run_optim(data, devs, param, dem, result_dict):
    ...
    result_dict = solve_model_and_extract_results(data=data, model=model, devs=devs, param=param, result_dict=result_dict)
...
def build_model(data, devs, param, dem):
    ...
    for d in model.clusters:
        for t in model.time_steps:
            ...
            model.constraints.add(model.power["PV", d, t] <= devs["PV"]["norm_power_clustered"][d][t] / 1000 * model.area["PV"])
            ...
            model.constraints.add(model.heat["STC", d, t] <= devs["STC"]["norm_power_clustered"][d][t] / 1000 * model.area["STC"])
    ...
    for d in model.clusters:
        for t in model.time_steps:
            ...
            heat_supply = sum(model.heat[dev, d, t] for dev in ["STC", "HP", "EB", "CHP", "BOI", "GHP", "BCHP", "BBOI", "WCHP", "WBOI", "FC"])
            ...
            power_supply = sum(model.power[dev, d, t] for dev in ["PV", "WT", "WAT", "CHP", "BCHP", "WCHP", "FC", "from_grid"])
            power_demand = dem["power"][d][t] + sum(model.power[dev, d, t] for dev in ["HP", "EB", "CC", "ELYZ", "to_grid"]) + model.ch["BAT", d, t]
    ...
    if param["peak_dem_met_conv"] == False:
        ...
        # Power
        model.constraints.add(model.cap["CHP"] + model.cap["BCHP"] + model.cap["WCHP"] + model.cap["FC"] + model.grid_limit_el >= param["peak_power"])
        ...
        model.constraints.add(model.cap["PV"] + model.cap["WT"] + model.cap["WAT"] + model.cap["CHP"] + model.cap["BCHP"] + model.cap["WCHP"] + model.cap["FC"] + model.grid_limit_el >= param["peak_power"])
    ...
    for dev in model.storage_devs:
        ...
        model.constraints.add(model.soc[dev, 0, 0] == soc_last * (1 - devs[dev]["sto_loss"]) ** dt + model.ch[dev, model.sigma[0], 0] * dt)

    ################################################################################
    # Grid limits
    ################################################################################

    model.constraints.add(model.from_gas_grid_total == dt * sum(model.gas["from_grid", d, t] * param["cluster_weights"][d] for d in model.clusters for t in model.time_steps))
    model.constraints.add(model.to_gas_grid_total == dt * sum(model.gas["to_grid", d, t] * param["cluster_weights"][d] for d in model.clusters for t in model.time_steps))
    model.constraints.add(model.from_el_grid_total == dt * sum(model.power["from_grid", d, t] * param["cluster_weights"][d] for d in model.clusters for t in model.time_steps))
    model.constraints.add(model.to_el_grid_total == dt * sum(model.power["to_grid", d, t] * param["cluster_weights"][d] for d in model.clusters for t in model.time_steps))
    model.constraints.add(model.biom_import_total == dt * sum(model.biom["import", d, t] * param["cluster_weights"][d] for d in model.clusters for t in model.time_steps))
    model.constraints.add(model.waste_import_total == dt * sum(model.waste["import", d, t] * param["cluster_weights"][d] for d in model.clusters for t in model.time_steps))
    model.constraints.add(model.hydrogen_import_total == dt * sum(model.hydrogen["import", d, t] * param["cluster_weights"][d] for d in model.clusters for t in model.time_steps))
...
def solve_model_and_extract_results(data, model, devs, param, result_dict):
    ...
    result_dict["total_ann_inv_cost"] = int(sum(safe_value(model.c_inv, k) for k in model.all_devs) + heat_grid_ann_costs)
    ...
    result_dict["co2_credit_feedin"] = int((safe_value_single(model.to_el_grid_total) * param["co2_el_feed_in"] + 
                                          safe_value_single(model.to_gas_grid_total) * param["co2_gas_feed_in"]) / 1000)
    ...
    result_dict["max_biom"] = int(max(safe_value(model.biom, ("import", d, t)) for d in model.clusters for t in model.time_steps))
    result_dict["max_waste"] = int(max(safe_value(model.waste, ("import", d, t)) for d in model.clusters for t in model.time_steps))
    result_dict["max_hydrogen"] = int(max(safe_value(model.hydrogen, ("import", d, t)) for d in model.clusters for t in model.time_steps))
    ...
    result_dict["PV_generation_uncl"] = [x / 1000 * safe_value(model.area, "PV") for x in devs["PV"]["norm_power"]]          # in kW
    result_dict["WT_generation_uncl"] = [x * safe_value(model.cap, "WT") for x in devs["WT"]["norm_power"]]                  # in kW
    result_dict["STC_generation_uncl"] = [x / 1000 * safe_value(model.area, "STC") for x in devs["STC"]["norm_power"]]       # in kW
    ...
    for k in ["TES", "CTES"]:
        cap_k = safe_value(model.cap, k)
        if cap_k > eps:
            result_dict[k]["vol_liter"] = round(cap_k / (param["c_w"] * param["rho_w"] * devs[k]["delta_T"]) * 3600 * 1000, 1)
        else:
    ...
    result_dict["total_co2_el_feed_in"] = int(safe_value_single(model.to_el_grid_total) * param["co2_el_feed_in"] / 1000) # t/a
    result_dict["total_co2_gas"] = int(safe_value_single(model.from_gas_grid_total) * param["co2_gas"] / 1000) # t/a
    result_dict["total_co2_gas_feed_in"] = int(safe_value_single(model.to_gas_grid_total) * param["co2_gas_feed_in"] / 1000) # t/a
    ...
    result_dict["total_co2_hydrogen"] = int(safe_value_single(model.hydrogen_import_total) * param["co2_hydrogen"] / 1000) # t/a
