...
def network_optimization2(data):
    """
    ...
    data, param = calc_flow(data, param, heat_loss_pipe=None, save_path=file_path)
    ...
    for i in range(max_iter):
        ...
        data, param = calc_flow(data, param, heat_loss_pipe=heat_loss_pipe, save_path=file_path)
...
def calc_flow(data, param, heat_loss_pipe=None, save_path=None):
    """
    Calculate the supply and return temperature and the flow rate in each pipe segment
    ...
    # list of clustered typical weeks
    # how many timesteps are there in a typical week
    time_steps = int(data.time["dataLength"] / data.time["timeResolution"])
    ...
    # waste heat profile
    wh_capacity = data.waste_heat_data["waste_heat_profile"]
    ...
    for i in data.pipeline.keys():
        ...
        for t in range(time_steps):
            ...
            if heat_loss_pipe == None:
                loss[i].append(0)
            else:
                loss[i].append(heat_loss_pipe[pipe_tuple][t])
    ...
    # create sets
    model.buildings = pyo.Set(initialize=BUILDINGS, doc="Set of buildings in the district")
    model.sources = pyo.Set(initialize=SOURCES, doc="Set of decentralized heat sources in the district")
    ...
    model.load_source_sink = pyo.Var(model.buildings, model.sources, model.t, within=pyo.NonNegativeReals,
    ...
    model.pipe_load_source = pyo.Var(model.sources, model.pipe, model.t, within=pyo.NonNegativeReals,
                                     doc="load on each pipe for each timestep coming from a source s")

    # load on each pipe for each timestep
    model.pipe_load = pyo.Var(model.pipe, model.t, within=pyo.NonNegativeReals,
 ...
 #   model.max_pipe_load = pyo.Var(model.pipe, within=pyo.NonNegativeReals, doc="maximal load on each pipe")
    ...
    model.max_pipe_flow_robust = pyo.Var(model.pipe, within=pyo.NonNegativeReals, doc="maximal load on each pipe with robust dimensioning")

    # heat produced by each heat source
    model.heat = pyo.Var(model.sources, model.t, within=pyo.Reals, doc="produced heat in kW")

    # heat loss per pipe/t (kW)
    model.heat_loss_pipe = pyo.Var(model.pipe, model.t, within=pyo.NonNegativeReals,
                                   doc="Heat loss per pipe, timestep (kW)")

    # heat loss per pipe/t (kW) allocated to a source
    model.heat_loss_pipe_source = pyo.Var(model.sources, model.pipe, model.t, within=pyo.NonNegativeReals,
    ...
    def demand_rule(model, building, t):
        return building_demand[building][t] == sum(
            (model.load_source_sink[building, source, t] - model.heat_loss_source_sink[building, source, t]) for source in model.sources)

    model.demand_rule = pyo.Constraint(model.buildings, model.t, rule=demand_rule)

    # 2) generated heat must be larger than supplied heat
    def capacity_rule(model, source, t):
        return model.heat[source, t] >= sum(
            model.load_source_sink[building, source, t] for building in model.buildings)

    model.capacity_rule = pyo.Constraint(model.sources, model.t, rule=capacity_rule)

    # 3) The heat of the waste heat source is assigned to the generated waste heat profile.
    def waste_heat_cap_rule(model, t):
        return model.heat["EH2", t] == wh_capacity[t]  # TODO statt EH2 WH

    model.wh_cap_rule = pyo.Constraint(model.t, rule=waste_heat_cap_rule)

    # 3) Generated heat by the energy hub equals the difference between the heat demand and the generated heat by the waste heat source (ensures that waste heat source is prioritized)
    def waste_heat_prio_rule1(model, t):
        return model.heat["EH2", t] >= sum(
            model.load_source_sink[building, source, t] for building in model.buildings for source in
            model.sources) - M * model.q[t]

    def waste_heat_prio_rule2(model, t):
        return model.heat["EH2", t] <= sum(
            model.load_source_sink[building, source, t] for building in model.buildings for source in
            model.sources) + M * (1 - model.q[t])

    def waste_heat_prio_rule3(model, t):
        return model.heat["EH1", t] <= M * model.q[t]

    def waste_heat_prio_rule4(model, t):
        return model.heat["EH1", t] <= sum(
            model.load_source_sink[building, source, t] for building in model.buildings for source in
            model.sources) - model.heat["EH2", t] + M * (1 - model.q[t])

    model.eh_cap_rule1 = pyo.Constraint(model.t, rule=waste_heat_prio_rule1)
    model.eh_cap_rule2 = pyo.Constraint(model.t, rule=waste_heat_prio_rule2)
    model.eh_cap_rule3 = pyo.Constraint(model.t, rule=waste_heat_prio_rule3)
    model.eh_cap_rule4 = pyo.Constraint(model.t, rule=waste_heat_prio_rule4)

    # 4) assigns the heat loss to each pipe
    def heat_loss_pipe_rule(model, pipe, t):
        return model.heat_loss_pipe[pipe, t] == loss[pipe][t]

    model.heat_loss_pipe_rule = pyo.Constraint(model.pipe, model.t, rule=heat_loss_pipe_rule)

    # 5) constraints to distribute heat_loss to each source sink combination
    def heat_loss_allocation_rule(model, pipe, t):  # constraint to evenly distribute the heat loss to each load_source_sink which flows through the given pipe
        return (sum(model.heat_loss_allocation[building, source, pipe, t] for building in model.buildings for source in model.sources if normalized(key_to_tuple(pipe)) in sorted_edges[(source, building)])
                == model.heat_loss_pipe[pipe, t])

    def heat_loss_source_sink_rule(model, building, source, t): # For each source–sink pair and timestep, the total heat loss is defined  as the sum of the allocated pipe-specific heat losses along the  corresponding network path between source and building.
        return model.heat_loss_source_sink[building, source, t] == sum(model.heat_loss_allocation[building, source, pipe, t] for pipe in model.pipe if normalized(key_to_tuple(pipe)) in sorted_edges[(source, building)])


    model.heat_loss_source_sink_rule = pyo.Constraint(model.buildings, model.sources, model.t, rule=heat_loss_source_sink_rule)
    model.heat_loss_allocation_rule = pyo.Constraint(model.pipe, model.t, rule=heat_loss_allocation_rule)

#    def heat_loss_pipe_rule1(model, pipe, t):
#        return model.heat_loss_pipe[pipe, t] <= model.pipe_load[pipe, t]

#    def heat_loss_pipe_rule2(model, pipe, t):
#        return model.heat_loss_pipe[pipe, t] >= model.pipe_load[pipe, t] - M * model.isflow[pipe, t]

#    def heat_loss_pipe_rule3(model, pipe, t):
#        return model.heat_loss_pipe[pipe, t] <= loss[pipe][t] + M * model.isflow[pipe, t]

#    def heat_loss_pipe_rule4(model, pipe, t):
#        return model.heat_loss_pipe[pipe, t] >= loss[pipe][t] - M*(1-model.isflow[pipe,t])

#    model.heat_loss_pipe_rule1 = pyo.Constraint(model.pipe, model.t, rule=heat_loss_pipe_rule1)
#    model.heat_loss_pipe_rule2 = pyo.Constraint(model.pipe, model.t, rule=heat_loss_pipe_rule2)
#    model.heat_loss_pipe_rule3 = pyo.Constraint(model.pipe, model.t, rule=heat_loss_pipe_rule3)
#    model.heat_loss_pipe_rule4 = pyo.Constraint(model.pipe, model.t, rule=heat_loss_pipe_rule4)


    # 6) assignment of flow for each source on each pipe
    def pipe_load_spec_rule(model, source, pipe, t):
        load_sum = 0
        load_reduction = 0
            ...
            if normalized(pipe_tuple) in {normalized(e) for e in path}:  # checks if the pipe is traversed
                ...
                for edge in path:
                    ...
                    if pipe_str1 in model.pipe:
                        load_reduction += model.heat_loss_allocation[building, source, pipe_str1, t]
                    else:
                        load_reduction += model.heat_loss_allocation[building, source, pipe_str2, t]

                load_sum += model.load_source_sink[building, source, t]


        return model.pipe_load_source[source, pipe, t] == (load_sum - load_reduction)

    model.pipe_load_spec_rule = pyo.Constraint(model.sources, model.pipe, model.t, rule=pipe_load_spec_rule)

    # 7) adds the pipe flows coming from each source
    def pipe_load_rule(model, pipe, t):
        return model.pipe_load[pipe, t] == sum(
            model.pipe_load_source[source, pipe, t] for source in model.sources)

    model.pipe_load_rule = pyo.Constraint(model.pipe, model.t, rule=pipe_load_rule)


#    # 8) Calculation of the maximal flow on each pipe
#    def max_load_rule(model, pipe, t):
#        return model.max_pipe_load[pipe] >= model.pipe_load[pipe, t]

#    model.max_load = pyo.Constraint(model.pipe, model.t, rule=max_load_rule)
    ...
    def n_2_rule1(model, pipe, t):
        return model.max_pipe_flow_robust[pipe] >= model.pipe_load[pipe, t] * 1000 / (c_f * deltaT[t] * rho_f)

    def n_2_rule2(model, pipe):
        return model.max_pipe_flow_robust[pipe] >= max_pipe_flow_eh1[pipe]

    model.n_2_rule1 = pyo.Constraint(model.pipe, model.t, rule=n_2_rule1)
    ...
    def flow_forward_rule(model, pipe, t):
        flow_reverse = 0
                ...
                for edge in path:
                    if normalized(edge) == normalized(
                        ...
                        flow_reverse += model.load_source_sink[
                            building, source, t]  # heat flows in the opposed direction of the defined flow direction, model.flow is
                        # added to the variable flow_reverse
        return flow_reverse <= M * (
                1 - model.y[pipe, t])  # if y is 0, the flow is allowed in the reverse direction

    def flow_reverse_rule(model, pipe, t):
        flow_forward = 0
            ...
            for building in model.buildings:
                ...
                for edge in path:
                    if normalized(edge) == normalized(pipe_tuple) and edge == pipe_tuple:
                        flow_forward += model.load_source_sink[building, source, t]

        return flow_forward <= M * model.y[
            pipe, t]  # if y is 1, the flow is allowed to flow in the forward direction

    model.flow_forward_rule = pyo.Constraint(model.pipe, model.t, rule=flow_forward_rule)
    model.flow_reverse_rule = pyo.Constraint(model.pipe, model.t, rule=flow_reverse_rule)
    ...
    for pipe in data.pipeline.keys():
        ...
        for source in SOURCES:
            ...
            pipe_load_source = []
            for t in range(time_steps):
                pipe_load.append(pyo.value(model.pipe_load[pipe, t]))
                pipe_load_source.append(pyo.value(model.pipe_load_source[source, pipe, t]))



            flow = np.array(pipe_load) * 1000 / (c_f * deltaT * rho_f)  # m³/s
            flow_max = float(np.max(flow))  # m³/s
            flow_source = np.array(pipe_load_source) * 1000 / (c_f * deltaT * rho_f)  # m³/s

            # dictionary also filled, only update the flow data
            data.pipeline[pipe].update({
                "flow": flow,
                "flow_robust": pipe_flow_robust,
                f"flow_{source}": flow_source,
                "flow_max": flow_max
            })
    ...
    for source in SOURCES:
        for building in BUILDINGS:
            loads = []
            for t in range(time_steps):
                loads.append(pyo.value(model.load_source_sink[building, source, t]))
            loads_max = np.max(loads)
            print(f"{source, building}: {loads_max}")
...
def optimization_diameter(data, param):
    ...
    def pump_pipe_d_rule(model, source, pipe, d, week, t):
        return model.pump_pipe_d_cand[source, pipe, d, week, t] - model.pump_pipe_d[source, pipe, d, week, t] <= M * (
                    1 - model.z[pipe, d])
    ...
    def p_station_rule(model, source, week, t):
        return model.p_station[source, week, t] == sum(
            model.flow_source_sink[building, source, week, t] for building in model.buildings) * dp_station_total / (
                    eta_pump * 1000)
    ...
    def pump_energy_total_rule(model):
        return model.pump_energy_total == sum(
            sum(model.pump_el[source, w, t] for source in model.sources for t in model.t) * data.clusterWeights[w] for w
            in model.week)
    ...
    for pipe in data.pipeline.keys():
        ...
        flow_source_max = {}
        for source in SOURCES:
            flow_source_max[source] = max(
                pyo.value(model.pipe_flow_source[source, pipe, week, t]) for week in data.clusters for t in
                range(time_steps))
            data.pipeline[pipe][f"flow_source_max_{source}"] = flow_source_max[source]

        # get the optimized maximal flow of each pipe segment
        data.pipeline[pipe]["flow_max"] = max(
            pyo.value(model.pipe_flow[pipe, week, t]) for week in data.clusters for t in range(time_steps))
    ...
    for pipe in data.pipeline.keys():
        ...
        print(
            f"Hier der minimale reale Durchmesser: {d_min_real} und hier der minimale approximierte Durchmesser: {d_min}")
        print(
            f"Hier der maximale reale Durchmesser: {d_max_real} und hier der maximale apporximierte Durchmesser: {d_max}")
...
def calc_diameter(data, param):
    ...
    for pipe_id, pipe in data.pipeline.items():
        ...
        if candidates:
            ...
            print(f"Hier sind die Kandidaten: {candidates} für Pipe: {pipe_id}")
            pipe["DN"] = best_DN
...
def output_diameter(data, param):
    ...
    for pipe_id, pipe in data.pipeline.items():
        ...
        for source in ["EH1", "EH2"]:
            flow_source = pipe[f"flow_{source}"]
    ...
    for source in ["EH1", "EH2"]:
        ...
        for pipe_id, pipe in data.pipeline.items():
            if pipe["from"] == source:
                Vdot_total_profile += pipe[f"flow_{source}"]  # m³/s
...
def calc_f_fric_per_pipe(data, param):
    ...
    for pid, pipe in data.pipeline.items():
        flow_max = pipe["flow_max"]  # m3/s
        print(pid)
        print(f"Hier ist der maximale Fluss: {flow_max}")
        DN = pipe["DN"]  # mm
        print(f"Hier ist der maximale DN: {DN}")
