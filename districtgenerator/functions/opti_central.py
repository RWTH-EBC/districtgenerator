...
def build_model(model, data, cluster):
    ...
    ################################################################################
    model.power_dom = pyo.Var(model.ecs_power, model.n, model.t, within=pyo.NonNegativeReals, doc="Electrical power to/from domestic devices")
    model.heat_dom = pyo.Var(model.ecs_heat, model.n, model.t, within=pyo.NonNegativeReals, doc="Heat to/from domestic devices")
    model.cool_dom = pyo.Var(model.ecs_cool, model.n, model.t, within=pyo.NonNegativeReals, doc="Cooling to/from domestic devices")
    model.gas_dom = pyo.Var(model.ecs_gas, model.n, model.t, within=pyo.NonNegativeReals, doc="Gas to/from domestic devices")
    ...
    # Storage variables
    model.soc_dom = pyo.Var(model.ecs_storage, model.n, model.t, within=pyo.NonNegativeReals, doc="State of charge of domestic storage devices") # State of charge of storage devices
    model.ch_dom = pyo.Var(model.ecs_storage, model.n, model.t, within=pyo.NonNegativeReals, doc="Charging power of domestic storage devices") # Charging power of storage devices
    model.dch_dom = pyo.Var(model.ecs_storage, model.n, model.t, within=pyo.NonNegativeReals, doc="Discharging power of domestic storage devices") # Discharging power of storage devices

    # Residual building demands (in kW)  [Sum for each building of all devices]
    model.res_dom_power = pyo.Var(model.n, model.t, within=pyo.NonNegativeReals, doc= "Residual power demand of buildings") # Only pos demand allowed
    model.res_dom_feed = pyo.Var(model.n, model.t, within=pyo.NonNegativeReals, doc="Residual feed-in of buildings") # Only pos feed-in allowed
    ...
    model.residual_power = pyo.Var(model.t, within=pyo.NonNegativeReals, doc="Residual power demand of the neighborhood") # Only pos demand allowed
    ...
    model.eh_power_from_grid = pyo.Var(model.t, within=pyo.NonNegativeReals, doc="Electricity imported from the grid by the Energy Hub")
    model.eh_power_to_grid = pyo.Var(model.t, within=pyo.NonNegativeReals, doc="Electricity exported to the grid by the Energy Hub")
    ...
    def create_eh_cool_capacity_constraint(device_name):
        ...
        def constraint_rule(model, t):
            if energyHubData == {}:
                ...
                return getattr(model, f"eh_cool_{device_name}")[t] <= energyHubData["capacities"][device_name]["cap"] * 1000  # kW to W
        return constraint_rule
    ...
    def create_eh_storage_soc_max_constraint(device_name):
        ...
        def constraint_rule(model, t):
            if energyHubData == {}:
                ...
                return getattr(model, f"eh_soc_{device_name}")[t] <= energyHubData["capacities"][device_name]["cap"] * 1000  # kWh to Wh
        return constraint_rule
    ...
    def create_dom_storage_ch_capacity_constraint(device_name):
        ...
        def constraint_rule(model, n, t):
            return model.ch_dom[device_name, n, t] <= buildingData[n]["capacities"][device_name] * param_dec_devs[device_name]["coeff_ch"]
        return constraint_rule

    def create_dom_storage_dch_capacity_constraint(device_name):
        """Factory-function für Storage Discharging Capacity Constraints"""
        def constraint_rule(model, n, t):
            return model.dch_dom[device_name, n, t] <= buildingData[n]["capacities"][device_name] * param_dec_devs[device_name]["coeff_ch"]
        return constraint_rule

    def create_dom_storage_soc_max_constraint(device_name):
        """Factory-function für Storage SOC Maximum Constraints"""
        def constraint_rule(model, n, t):
            return model.soc_dom[device_name, n, t] <= param_dec_devs[device_name]["soc_max"] * buildingData[n]["capacities"][device_name]
        return constraint_rule

    def create_dom_storage_soc_min_constraint(device_name):
        """Factory-function für Storage SOC Minimum Constraints"""
        def constraint_rule(model, n, t):
            return model.soc_dom[device_name, n, t] >= param_dec_devs[device_name]["soc_min"] * buildingData[n]["capacities"][device_name]
        return constraint_rule
    ...
    def cc_building_conversion_rule(model, n, t):
        return model.cool_dom["CC", n, t] == model.power_dom["CC", n, t] * param_dec_devs["CC"]["grade"] * (273.15 + 5) / (T_e[t] - 5)
 
    # Constraints for building devices conversion
    model.hp_heat_balance = pyo.Constraint(model.n, model.t, rule=hp_heat_balance_rule, doc="Heat pump total heat output balance between HP35 and HP55 modes")
    model.hp_power_balance = pyo.Constraint(model.n, model.t, rule=hp_power_balance_rule, doc="Heat pump total power consumption balance between HP35 and HP55 modes")
    model.hp_mode_new = pyo.Constraint(model.n, model.t, rule=hp_mode_constraint_new_rule, doc="Heat pump mode restriction for new buildings (≥1995): only HP35 mode allowed")
    model.hp_mode_old = pyo.Constraint(model.n, model.t, rule=hp_mode_constraint_old_rule, doc="Heat pump mode restriction for old buildings (<1995): only HP55 mode allowed")
    model.hp35_conversion = pyo.Constraint(model.n, model.t, rule=hp35_conversion_rule, doc="Heat pump HP35 mode: converts electricity to heat with COP dependent on ambient temperature (35°C supply)")
    model.hp55_conversion = pyo.Constraint(model.n, model.t, rule=hp55_conversion_rule, doc="Heat pump HP55 mode: converts electricity to heat with COP dependent on ambient temperature (55°C supply)")
    model.eh_conversion = pyo.Constraint(model.n, model.t, rule=eh_conversion_rule, doc="Electric heater: converts electricity to heat for space heating and DHW")
    model.chp_heat_conversion = pyo.Constraint(model.n, model.t, rule=chp_heat_conversion_rule, doc="CHP thermal conversion: gas to heat with thermal efficiency")
    model.chp_power_conversion = pyo.Constraint(model.n, model.t, rule=chp_power_conversion_rule, doc="CHP electrical conversion: gas to electricity with electrical efficiency")
    model.boiler_conversion = pyo.Constraint(model.n, model.t, rule=boiler_conversion_rule, doc="Gas boiler conversion: natural gas to heat with thermal efficiency")
    ...
    def identify_ev_parking_periods(buildingData, EV_dem, time_steps, dt) -> dict:
        ...
        2. For other buildings (e.g., offices, commercial): parking time is 7 hours
        """
        parking_periods = {}
    ...
    def ev_binary1_rule(model, n, t):
        return model.dch_dom["EV", n, t] <= model.binary_EV[n, t] * BIG_M
    
    def ev_binary2_rule(model, n, t):
        return model.ch_dom["EV", n, t] <= (1 - model.binary_EV[n, t]) * BIG_M
    ...
    def hline_binary1_rule(model, n, t):
        return model.res_dom_power[n, t] <= model.binary_HLINE[n, t] * BIG_M

    def hline_binary2_rule(model, n, t):
        return model.res_dom_feed[n, t] <= (1 - model.binary_HLINE[n, t]) * BIG_M
    ...
    def create_eh_storage_binary1_constraint(device_name):
        ...
        def constraint_rule(model, t):
            if energyHubData == {}:
                ...
                return getattr(model, f"eh_dch_{device_name}")[t] <= getattr(model, f"eh_binary_{device_name}")[t] * BIG_M
        return constraint_rule
    ...
    def create_eh_storage_binary2_constraint(device_name):
        ...
        def constraint_rule(model, t):
            if energyHubData == {}:
                ...
                return getattr(model, f"eh_ch_{device_name}")[t] <= (1 - getattr(model, f"eh_binary_{device_name}")[t]) * BIG_M
        return constraint_rule
    ...
    def electricity_balance_rule(model, n, t):
        return (model.res_dom_power[n, t] + model.power_dom["PV", n, t] + model.power_dom["CHP", n, t] + model.dch_dom["BAT", n, t] +
                model.dch_dom["EV", n, t]
                == model.power_dom["Elec_dem", n, t] + model.ch_dom["EV", n, t] + model.power_dom["HP", n, t] + model.power_dom[
    ...
    model.electricity_balance = pyo.Constraint(model.n, model.t, rule=electricity_balance_rule, doc="Electricity balance for each building")
    model.heating_balance = pyo.Constraint(model.n, model.t, rule=heating_balance_rule, doc="Heating balance for each building")
    model.cooling_balance = pyo.Constraint(model.n, model.t, rule=cooling_balance_rule, doc="Cooling balance for each building")
    ...
    def eh_heating_balance_rule(model, t):
        return (model.eh_heat_STC[t] + model.eh_heat_HP[t] + model.eh_heat_EB[t] + model.eh_heat_CHP[t]
                ...
                + model.eh_heat_WCHP[t] + model.eh_heat_WBOI[t] + model.eh_heat_FC[t] + model.eh_dch_TES[t] # Heat supply 
    ...
    def eh_heat_supply_rule(model, t):
        return model.eh_heat_to_grid[t] >= sum(model.heat_dom["heat_grid", n, t] for n in model.n) + network_losses_heating[t]
    
    # The EH must supply the cooling demand of the buildings connected to the grid
    def eh_cool_supply_rule(model, t):
        return model.eh_cool_to_grid[t] >= sum(model.cool_dom["heat_grid", n, t] for n in model.n) + network_losses_cooling[t]
    ...
    def eh_electricity_balance_rule(model, t):
        return (model.eh_power_PV[t] + model.eh_power_WT[t] + model.eh_power_WAT[t] + model.eh_power_CHP[t]
                + model.eh_power_BCHP[t] + model.eh_power_WCHP[t] + model.eh_power_FC[t] + model.eh_dch_BAT[t] + model.eh_power_from_grid[t]
    ...
    def eh_gas_balance_rule(model, t):
        return (model.eh_gas_from_grid[t] + model.eh_gas_SAB[t] + model.eh_dch_GS[t] # Gas supply
                == model.eh_gas_CHP[t] + model.eh_gas_BOI[t] + model.eh_gas_GHP[t] + model.eh_ch_GS[t] + model.eh_gas_to_grid[t]) # Gas demand
    ...
    def eh_hydrogen_balance_rule(model, t):
        return (model.eh_hydrogen_ELYZ[t]  + model.eh_hydrogen_from_neighborhood[t] + model.eh_dch_H2S[t]
                == model.eh_hydrogen_FC[t] + model.eh_hydrogen_SAB[t] + model.eh_ch_H2S[t] + model.eh_hydrogen_to_neighborhood[t])
    ...
    def trafo_binary1_rule(model, t):
        return model.power_from_grid[t] <= model.yTrafo[t] * BIG_M
    
    def trafo_binary2_rule(model, t):
        return model.power_to_grid[t] <= (1 - model.yTrafo[t]) * BIG_M
    
    model.neighborhood_elec_balance = pyo.Constraint(model.t, rule=neighborhood_elec_balance_rule, doc="Power_balance_neighborhood")
    ...
    def neighborhood_hydrogen_balance_rule(model, t):
        return (model.power_hydrogen_grid_import[t] + model.eh_hydrogen_to_neighborhood[t] == model.eh_hydrogen_from_neighborhood[t] 
    ...
    model.daily_peak_constraints = pyo.Constraint(model.days, model.t_day, rule=daily_peak_basic_rule, doc="Daily_peak_rule")
    model.daily_peak_selection = pyo.Constraint(model.days, rule=daily_peak_selection_rule, doc="Daily_peak_selection")
    model.daily_peak_upper_bound = pyo.Constraint(model.days, model.t, rule=daily_peak_upper_bound_rule, doc="Daily_peak_upper_bound")
    model.daily_peak_lower_bound = pyo.Constraint(model.days, model.t, rule=daily_peak_lower_bound_rule, doc="Daily_peak_lower_bound")
...
def solve_model_and_extract_results(model, data):
    ...
    helper_func_extract_eh_results(model=model, results_dict=results_dict, variable_type="eh_power", device_set=EH_ECS_POWER, time_steps=time_steps)
    helper_func_extract_eh_results(model=model, results_dict=results_dict, variable_type="eh_heat", device_set=EH_ECS_HEAT, time_steps=time_steps)
    helper_func_extract_eh_results(model=model, results_dict=results_dict, variable_type="eh_cool", device_set=EH_ECS_COOL, time_steps=time_steps)
    helper_func_extract_eh_results(model=model, results_dict=results_dict, variable_type="eh_gas", device_set=EH_ECS_GAS, time_steps=time_steps)
    helper_func_extract_eh_results(model=model, results_dict=results_dict, variable_type="eh_hydrogen", device_set=EH_ECS_HYDROGEN, time_steps=time_steps)
    helper_func_extract_eh_results(model=model, results_dict=results_dict, variable_type="eh_biom", device_set=EH_ECS_BIOMASS, time_steps=time_steps)
    helper_func_extract_eh_results(model=model, results_dict=results_dict, variable_type="eh_waste", device_set=EH_ECS_WASTE, time_steps=time_steps)

    # Storage results
    helper_func_extract_eh_results(model=model, results_dict=results_dict, variable_type="eh_ch", device_set=EH_ECS_STORAGE, time_steps=time_steps)
    helper_func_extract_eh_results(model=model, results_dict=results_dict, variable_type="eh_dch", device_set=EH_ECS_STORAGE, time_steps=time_steps)
    helper_func_extract_eh_results(model=model, results_dict=results_dict, variable_type="eh_soc", device_set=EH_ECS_STORAGE, time_steps=time_steps)
    ...
    for n in range(nbuildings):
        ...
        for t in time_steps:
            ...
            gas_total = pyo.value(model.gas_dom["BOI", n, t]) + pyo.value(model.gas_dom["CHP", n, t]) #! This should not be here -> Doubling of Code possible
            ...
            hydrogen_total = pyo.value(model.hydrogen_dom["H2BOI", n, t]) + pyo.value(model.hydrogen_dom["FC", n, t]) #! This should not be here -> Doubling of Code possible
