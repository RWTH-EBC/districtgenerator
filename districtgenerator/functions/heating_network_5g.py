...
The Energy Hub warm pipe und cold pipe temperature is fixed in this first model. 

Decentral heatpumps in every connected building boost the supply temperature to the needed level, direct coolers supply the needed cooling.

Pipe heat losses are calculated afterward from the local pipe temperatures.
In this simplified model, pipe heat losses or gains do not change the temperatures along the pipes.

The model workflow is:
    1) prepare building loads and temperature requirements
    2) calculate building mass flows
    3) aggregate pipe massflows for every time step
...
def network_5G(data, compute_costs=True, save_debug=True):
    """
    Simple auto-only 2-pipe network model for 5G bidirectional networks.
    ...
    plot_random_pipe_massflow_layouts_5g(
        data,
        param,
        n_times=5,
        seed=42,
        only_active_timesteps=True
    )

    return data, param


def add_test_cooling_profiles_5g(
        data,
        target_central_massflow_kg_s=2.5,
        cooling_peak_factor=1.0,
        summer_peak_position=0.58,
        summer_width=0.13,
        seed=42
    ...
    """
    Add artificial cooling profiles to all heat_grid buildings for testing
    the 5G fixed-temperature model.

    The total cooling peak is scaled so that the theoretical central
    cooling mass flow is approximately target_central_massflow_kg_s.

    Assumption:
        Q_cooling_to_network = Q_cooling

    Therefore:
        m_cool = Q_cooling * 1000 / (c_f * deltaT_5g)

    Parameters
    ----------
    target_central_massflow_kg_s : float
        Desired approximate total cooling mass flow at peak.

    cooling_peak_factor : float
        Scaling factor. Use > 1.0 if cooling should clearly dominate DHW
        and create negative signed root flow.

    summer_peak_position : float
        Position of the summer peak in the year.
        0.5 = middle of year, 0.58 ≈ late July / early August.

    summer_width : float
        Width of the seasonal cooling profile.
    ...
    def _as_profile(value, name):
        ...
        if arr.shape == ():
            arr = np.full(T_len, float(arr), dtype=float)
    ...
    T_warm = _as_profile(
        data.heat_grid_data.get("T_warm_5G", 20.0),
        "T_warm_5G"
    )

    T_cold = _as_profile(
        data.heat_grid_data.get("T_cold_5G", 10.0),
        "T_cold_5G"
    )

    deltaT_5g = T_warm - T_cold

    if np.any(deltaT_5g <= 1e-9):
        raise ValueError(
            "Cannot create test cooling profile: "
            "T_warm_5G must be larger than T_cold_5G."
        )

    # Use mean deltaT for the test sizing.
    deltaT_design = float(np.mean(deltaT_5g))

    Q_cooling_peak_total_kW = (
            target_central_massflow_kg_s
            * c_f
            * deltaT_design
            / 1000.0
            * cooling_peak_factor
    ...
    # ------------------------------------------------------------------
    # Create normalized cooling shape
    # ------------------------------------------------------------------
    x = np.linspace(0.0, 1.0, T_len)

    seasonal_shape = np.exp(
        -0.5 * ((x - summer_peak_position) / summer_width) ** 2
    )

    # Cut very small winter values
    seasonal_shape = np.where(seasonal_shape < 0.05, 0.0, seasonal_shape)

    # Daily shape if hourly profile is used
    if T_len % 24 == 0:
        hour_of_day = np.arange(T_len) % 24

        # Cooling mainly between 7:00 and 21:00, peak around afternoon
        daily_shape = np.sin(
            np.pi * np.clip((hour_of_day - 7.0) / 14.0, 0.0, 1.0)
        )

        daily_shape = np.maximum(daily_shape, 0.0)

    else:
        daily_shape = np.ones(T_len, dtype=float)

    profile_shape = seasonal_shape * daily_shape

    if np.max(profile_shape) <= 1e-12:
        raise ValueError("Generated cooling profile shape is zero everywhere.")

    profile_shape = profile_shape / np.max(profile_shape)

    # ------------------------------------------------------------------
    # Connected buildings and distribution weights
    # ------------------------------------------------------------------
    buildings_connected = [
    ...
    if not buildings_connected:
        raise ValueError(
            "No heat_grid buildings found for test cooling profile."
        )
    ...
    for building in buildings_connected:
        ...
        if "bes_obj" in building:
            heat_design = float(
                getattr(building["bes_obj"], "design_load_heating", 0.0)
            )
            dhw_design = float(
                getattr(building["bes_obj"], "design_load_dhw", 0.0)
            )
            weight = max(heat_design + dhw_design, 1.0)

        else:
            heat = np.asarray(building["user"].heat, dtype=float)
            dhw = np.asarray(building["user"].dhw, dtype=float)
            weight = max(float(np.sum(heat + dhw)), 1.0)
    ...
    # Small variation so not all buildings have identical relative cooling
    variation = rng.uniform(0.85, 1.15, size=len(buildings_connected))
    weights = weights * variation
    weights = weights / np.sum(weights)

    # ------------------------------------------------------------------
    # Assign cooling profiles to buildings
    # ------------------------------------------------------------------
    total_cooling_profile_kW = np.zeros(T_len, dtype=float)

    for building, weight in zip(buildings_connected, weights):
        ...
        # load_parameter_5g_fixed expects building["user"].cooling in W,
        # because it divides by 1000 later.
        building["user"].cooling = cooling_profile_kW * 1000.0

        total_cooling_profile_kW += cooling_profile_kW

    # ------------------------------------------------------------------
    # Store debug values
    # ------------------------------------------------------------------
    theoretical_massflow_profile = (
            total_cooling_profile_kW * 1000.0 / (c_f * deltaT_5g)
    )

    data.heat_grid_data["test_cooling_profile_total_kW"] = total_cooling_profile_kW
    data.heat_grid_data["test_cooling_massflow_theoretical_kg_s"] = theoretical_massflow_profile

    print("[5G test cooling]")
    print(f"Connected buildings: {len(buildings_connected)}")
    print(f"Target central cooling massflow: {target_central_massflow_kg_s:.3f} kg/s")
    print(f"Cooling peak factor: {cooling_peak_factor:.3f}")
    print(f"Mean deltaT_5g: {deltaT_design:.3f} K")
    print(f"Total cooling peak: {np.max(total_cooling_profile_kW):.3f} kW")
    print(f"Theoretical peak cooling massflow: {np.max(theoretical_massflow_profile):.3f} kg/s")
...
def load_parameter_5g_fixed(data):
    ...
    and fixed warm/cold rail temperatures for the first 5G model.
    ...
    - Cooling mode:
          cold pipe -> direct cooler -> warm pipe
    - Building substations are assumed to return exactly to the opposite
      fixed rail temperature.
    - Pipe heat losses/gains are calculated later and do not feed back into
      rail temperatures in this first model.

    This method is structured so that a later fixed-point model can replace
    only the local node temperature fields and the building exchange step.
    """
    ...
    # ------------------------------------------------------------------
    # Fixed 5G rail temperatures and basic assumptions
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # HX design assumptions and decentral heat pump COP assumptions
    # ------------------------------------------------------------------

    dT_HX_sup_SH = float(heat_grid_data.get("dT_HX_sup_SH", 4.0))
    dT_HX_ret_SH_design = float(heat_grid_data.get("dT_HX_ret_SH_design", 3.0))

    dT_HX_sup_DHW = float(heat_grid_data.get("dT_HX_sup_DHW", 4.0))
    dT_HX_ret_DHW_min = float(heat_grid_data.get("dT_HX_ret_DHW_min", 10.0))

    T_dhw_required = float(data.decentral_device_data["TES_DHW"]["T_DHW_needed"])
    T_cold_water = float(heat_grid_data.get("T_cold_water", 10.0))

    HX_UA_safety_factor = float(heat_grid_data.get("HX_UA_safety_factor", 1.0))

    # Cooling temperature assumptions for direct cooling HX on building side
    T_cooling_supply_sec = float(heat_grid_data.get("T_cooling_supply_sec", 18.0))
    T_cooling_return_sec = float(heat_grid_data.get("T_cooling_return_sec", 24.0))


    hp_data = data.decentral_device_data.get("HP", {})
    eta_hp_carnot = float(heat_grid_data.get("eta_decentral_HP_carnot",hp_data.get("eta_carnot", 0.45)))
    COP_min = float(heat_grid_data.get("COP_decentral_HP_min", 1.01))
    COP_max = float(heat_grid_data.get("COP_decentral_HP_max", 8.0))
    min_lift_K = float(heat_grid_data.get("min_HP_lift_K", 3.0))
    ...
    def calc_decentral_hp_cop_heating(T_sink_C, T_warm_in_C, T_cold_out_C):     #Todo optional: durch besseres COP Modell austauschen
        ...
        if T_sink_C.shape == (): #Kann so sowohl mit Skalaren als auch mit bereits existierenden Arrays umgehen
        ...
        # Approximation of the source temperature
        T_source_C = 0.5 * (T_warm_in_C + T_cold_out_C)

        T_source_K = T_source_C + 273.15
        T_sink_K = T_sink_C + 273.15

        lift = np.maximum(T_sink_K - T_source_K, min_lift_K)

        COP = eta_hp_carnot * T_sink_K / lift
        COP = np.clip(COP, COP_min, COP_max)

        return COP

    # fast lookup from the old code
    node_lookup = {
    ...
    T_boost_supply_SH_by_node = {}
    T_boost_supply_DHW_by_node = {}
    ...
    # prepared for later iterative model
    T_warm_in_building_5g = {}
    T_cold_out_heating_5g = {}

    T_cold_in_building_5g = {}
    T_warm_out_cooling_5g = {}

    # Local node temperatures
    # In the fixed model all nodes have the same temperatures
    # In the later fixed-point model these fields will be overwritten
    T_warm_node_5g = {}
    T_cold_node_5g = {}

    for node in data.pipeline_nodes.keys():
        T_warm_node_5g[node] = T_warm_fixed.copy()
        T_cold_node_5g[node] = T_cold_fixed.copy()
    ...
    invalid_5g_heaters = [building["buildingFeatures"]["heater"] for building in data.district if building["buildingFeatures"]["heater"] in ("heat_grid_OEB", "heat_grid_BEB", "heat_grid_BHP")]
    ...
    for building in data.district:
        ...
        # Needed supply temperatures
        T_boost_SH = Ts_sec_SH + dT_HX_sup_SH
        T_boost_DHW = np.full(T_len, T_dhw_required + dT_HX_sup_DHW, dtype=float)
        ...
        # Useful heat loads including substation losses.
        Q_SH_raw = sh_load * (1.0 + h_loss_subst / 100.0)
        Q_DHW_raw = dhw_load * (1.0 + h_loss_subst / 100.0)

        if use_STC_in_5g_loads:
            generation_STC = np.asarray(building["generationSTC"], dtype=float) / 1000.0

            # STC is used first for the demand with the lower required temperature
            SH_is_lower_temp = T_boost_SH <= T_boost_DHW

            Q_SH = Q_SH_raw.copy()
            Q_DHW = Q_DHW_raw.copy()

            stc_to_SH = np.where(
                SH_is_lower_temp,
                np.minimum(generation_STC, Q_SH),
                np.maximum(generation_STC - Q_DHW, 0.0)
            )

            stc_to_DHW = np.where(
                SH_is_lower_temp,
                np.maximum(generation_STC - Q_SH, 0.0),
                np.minimum(generation_STC, Q_DHW)
            )

            Q_SH = np.maximum(Q_SH - stc_to_SH, 0.0)
            Q_DHW = np.maximum(Q_DHW - stc_to_DHW, 0.0)

        else:
            Q_SH = Q_SH_raw
            Q_DHW = Q_DHW_raw

        Q_heat_useful = Q_SH + Q_DHW
        ...
        COP_DHW = calc_decentral_hp_cop_heating(
            T_sink_C=T_boost_DHW,
            T_warm_in_C=T_warm_fixed,
            T_cold_out_C=T_cold_fixed
        )

        # Electric power consumption
        P_HP_SH = np.divide(Q_SH, COP_SH, out=np.zeros_like(Q_SH))
        P_HP_DHW = np.divide(Q_DHW, COP_DHW, out=np.zeros_like(Q_DHW))


        # Heat extracted from 5G network
        Q_from_network_SH = Q_SH - P_HP_SH
        Q_from_network_DHW = Q_DHW - P_HP_DHW

        Q_heat_from_network = Q_from_network_SH + Q_from_network_DHW
        P_HP_decentral = P_HP_SH + P_HP_DHW


        dT_min_FRC = float(
            heat_grid_data.get("dT_min_FRC", 2.0)
        )

        direct_cooling_possible = (
                (T_cold_fixed + dT_min_FRC < T_cooling_supply_sec)
                & (T_warm_fixed + dT_min_FRC < T_cooling_return_sec)
        )

        infeasible_cooling = ((Q_cooling > 1e-9) & (~direct_cooling_possible))

        if np.any(infeasible_cooling):
            bad_idx = np.where(infeasible_cooling)[0][:20]
            raise ValueError(
                "Direct cooling is not feasible for all cooling timesteps. "
                f"First bad indices: {bad_idx}. "
                "Lower T_warm_5G/T_cold_5G or add a backup cooling device."
            )
        ...
        # SH heat exchanger
        if np.any(Q_SH > 0.0):
            Q_SH_design_W = (building["bes_obj"].design_load_heating*(1.0 + h_loss_subst / 100.0))
        ...
        # Cooling direct heat exchanger
        if np.any(Q_cooling > 0.0):
            Q_COOL_design_W = float(np.max(Q_cooling)) * 1000.0

            # Hot side: building cooling loop
            T_hot_in_design = float(T_cooling_return_sec)
            T_hot_out_design = float(T_cooling_supply_sec)

            # Cold side: 5G cold rail -> 5G warm rail
            T_cold_in_design = float(np.max(T_cold_fixed))
            T_cold_out_design = float(np.max(T_warm_fixed))

            if T_hot_in_design <= T_cold_out_design or T_hot_out_design <= T_cold_in_design:
                raise ValueError(
                    "Direct cooling HX infeasible. "
                    f"Got hot_in={T_hot_in_design}, "
                    f"hot_out={T_hot_out_design}, "
                    f"cold_in={T_cold_in_design}, "
                    f"cold_out={T_cold_out_design}."
                )

            UA_COOL = HX_UA_safety_factor * hx_calc_UA_design(
                Q_design_W=Q_COOL_design_W,
                T_primary_in_design=T_hot_in_design,
                T_primary_out_design=T_hot_out_design,
                T_secondary_in_design=T_cold_in_design,
                T_secondary_out_design=T_cold_out_design
            )
        else:
            UA_COOL = 0.0
        ...
        T_boost_supply_SH_by_node[node_key] = T_boost_SH
        T_boost_supply_DHW_by_node[node_key] = T_boost_DHW
        ...
        UA_COOL_by_node[node_key] = UA_COOL

        # Fixed rail values at building interface prepared for iterative model
        T_warm_in_building_5g[node_key] = T_warm_fixed.copy()
        T_cold_out_heating_5g[node_key] = T_cold_fixed.copy()

        T_cold_in_building_5g[node_key] = T_cold_fixed.copy()
        T_warm_out_cooling_5g[node_key] = T_warm_fixed.copy()
    ...
    # Store into param
    # ------------------------------------------------------------------
    param["T_warm_node_5g"] = T_warm_node_5g
    param["T_cold_node_5g"] = T_cold_node_5g
    ...
    param["T_boost_supply_SH_by_node"] = T_boost_supply_SH_by_node
    param["T_boost_supply_DHW_by_node"] = T_boost_supply_DHW_by_node
    ...
    param["UA_COOL_by_node"] = UA_COOL_by_node

    param["T_warm_in_building_5g"] = T_warm_in_building_5g
    param["T_cold_out_heating_5g"] = T_cold_out_heating_5g
    param["T_cold_in_building_5g"] = T_cold_in_building_5g
    param["T_warm_out_cooling_5g"] = T_warm_out_cooling_5g
    ...
    # Compatibility aliases for downstream functions during migration.
    # Do not use these aliases as final 5G semantics.
    param["Q_by_node"] = Q_5g_signed_by_node
    param["net_heat_demand"] = np.maximum(net_thermal_balance_5g, 0.0)
    param["net_cooling_demand"] = np.maximum(-net_thermal_balance_5g, 0.0)
    ...
    # Decentral HP annualization factor for later network/device costing
    hp_lifetime = data.decentral_device_data.get("HP", {}).get("life_time", None)

    if hp_lifetime is not None:
        param["decentral_HP_ann_factor"] = calc_annual_factor(data, hp_lifetime)
...
def update_pipe_data_selection(data, param, Ts_max):
    ...
    if Ts_max > 60.0:
        ...
        print("Rohrtyp PE wurde ausgewählt!")
...
def calc_flow_5g_fixed(data, param):
    ...
    # For 5G bidirectional flow this should normally be 0.
    # If used, it is applied only when the corresponding load is active.
    ...
    for n in param["Q_5g_signed_by_node"].keys():
        ...
        T_warm_in_heat = np.asarray(param["T_warm_in_building_5g"].get(n, T_warm_fixed), dtype=float)
        T_cold_out_heat = np.asarray(param["T_cold_out_heating_5g"].get(n, T_cold_fixed), dtype=float)

        T_cold_in_cool = np.asarray(param["T_cold_in_building_5g"].get(n, T_cold_fixed), dtype=float)
        T_warm_out_cool = np.asarray(param["T_warm_out_cooling_5g"].get(n, T_warm_fixed), dtype=float)

        deltaT_heat = T_warm_in_heat - T_cold_out_heat
        deltaT_cool = T_warm_out_cool - T_cold_in_cool
        ...
        # Optional minimum flow fraction.
        # Unlike in the 2-pipe model, do not force flow in inactive timesteps.
        ...
        active_heat = Q_heat_eff > 0.0
        active_cool = Q_cool_eff > 0.0

        m_heat = np.where(active_heat, np.maximum(m_heat_raw, alpha * m_heat_max), 0.0)
        m_cool = np.where(active_cool, np.maximum(m_cool_raw, alpha * m_cool_max), 0.0)
...
def calc_diameter_5g_fixed(data, param):
    ...
    for pipe_id, pipe in data.pipeline.items():
        ...
        if V_max <= 1e-9:
            print(f"Warnung: Rohr mit id {pipe_id} wird zu keinen Zeitpunkt verwendet!")
        ...
        pipe["d_i_m"] = pipe["d_i"] / 1000.0

        #pipe["v_design"] = float(v_selected)
        #pipe["dp_per_m_design"] = float(dp_per_m_selected)
...
def calc_heat_loss_pipe_5g_fixed(data, param):
    ...
    def _get_pipe_temperature(pipe, rail, fallback):
        """
        Return pipe temperature profile.

        Priority:
        1. pipe["T_warm"] / pipe["T_cold"]
        2. mean of pipe["T_warm_in"] and pipe["T_warm_out"]
        3. fixed fallback temperature
        """

        key = f"T_{rail}"
        key_in = f"T_{rail}_in"
        key_out = f"T_{rail}_out"

        #already up to date for iterative algorithm
        if key_in in pipe and key_out in pipe:
            T = 0.5 * (np.asarray(pipe[key_in], dtype=float) + np.asarray(pipe[key_out], dtype=float))

        elif key in pipe:
            T = np.asarray(pipe[key], dtype=float)

        else:
            T = np.asarray(fallback, dtype=float)

        if len(T) != T_len:
            raise ValueError(
                f"Pipe temperature {key} has length {len(T)}, "
                f"but expected {T_len}."
            )
    ...
    for pid, pipe in data.pipeline.items():
        ...
        T_warm = _get_pipe_temperature(
            pipe=pipe,
            rail="warm",
            fallback=T_warm_default
        )

        T_cold = _get_pipe_temperature(
            pipe=pipe,
            rail="cold",
            fallback=T_cold_default
        )

        Q_warm = UA * (T_warm - T_soil) / 1000.0  # kW
        Q_cold = UA * (T_cold - T_soil) / 1000.0  # kW
        ...
        # Compatibility name for old plotting/debug methods
...
def compute_pump_power_5g_fixed(data, param):
    ...
    Hydraulic losses depend on abs(flow), not on signed flow.
    """
    ...
    pair_to_pid = {
        (p["from"], p["to"]): pid
        for pid, p in data.pipeline.items()
    }
    ...
    # Optional debug arrays per pipe
    for pid, pipe in data.pipeline.items():
    ...
    for t in range(T_len):
        ...
        for pid, pipe in data.pipeline.items():
            ...
            if Re <= 1e-9:
                print("Warnung: berechnete Reynolds Zahl ist 0!")
                f = 0.0
            else:
                f = fluids.friction.friction_factor(Re=Re, eD=rough / d_i)
...
def plot_network_results_5g_fixed(data, param):
    ...
    _finish_plot(
        ...
        f"pipeline_id_5g_{data.scenario_name}"
    )
    ...
    _finish_plot(
        ...
        ax,
        "5G network - diameter",
        f"pipeline_diameter_5g_{data.scenario_name}"
    )
    ...
    _finish_plot(
        ...
        f"pipeline_velocity_max_5g_{data.scenario_name}"
    )
    ...
    _finish_plot(
        ...
        f"pipeline_pressure_drop_max_5g_{data.scenario_name}"
    )
    ...
    _finish_plot(
        ...
        f"pipeline_energy_density_5g_{data.scenario_name}"
    )

    # -------------------------------------------------------------------------
    # 6) Pipeline map by heat-exchange density
    # -------------------------------------------------------------------------
    for pipe_id, pipe in data.pipeline.items():
        length = max(float(pipe["length"]), 1e-12)
    ...
    _finish_plot(
        fig,
        ax,
        "5G network - heat exchange density (MWh/m)",
        f"pipeline_heat_exchange_density_5g_{data.scenario_name}"
    )
...
def compute_and_save_network_costs_5g_fixed(data, param):
    ...
    for building in buildings_connected:
        ...
        # separate decentralized HP/heating side and direct-cooling side.
        substation_capacity = heat_cap + cool_cap
    ...
    for pipe in data.pipeline.values():
        ...
        # 5G has warm and cold pipe.
        inv_pipes += (
                length
                * param["pipe_dict"][DN]["Pipe Cost (€/m)"]
                * 2.0
        )

        # Construction/trench cost only once per segment.
        inv_construction += (
                length
                * param["pipe_dict"][DN]["Construction Cost (€/m)"]
        )

    pipe_ann_factor = data.heat_grid_data["pipe"]["pipe_ann_factor"]

    pipes_ann_costs = (
                              inv_pipes + inv_construction
                      ) * pipe_ann_factor

    pipes_om_costs = (
                             inv_pipes + inv_construction
                     ) * data.heat_grid_data["pipe"]["cost_om_pipe"]
    ...
    pump_ann_costs = (
            inv_pump
            * data.heat_grid_data["pump"]["pump_ann_factor"]
    )

    pump_om_costs = (
            inv_pump
            * data.heat_grid_data["pump"]["cost_om_pump"]
    )

    # P_pump is W. With hourly timesteps:
    # sum(W) / 1000 = kWh/a
    pump_energy_total = (
            float(np.sum(data.heat_grid_data["P_pump"])) / 1000.0
    )

    pump_electricity_costs = (
            pump_energy_total
            * data.ecoData["price_supply_el_eh"][0]
    )
    ...
    base = os.path.join(
        param["dir_result"],
        f"network_cost_stack_5g_{data.scenario_name}"
    )
    ...
    # ------------------------------------------------------------------
    # Temperature debug plot: warm/cold/soil
    # ------------------------------------------------------------------
    T_warm = np.asarray(param["T_warm_fixed_5g"], dtype=float)
    T_cold = np.asarray(param["T_cold_fixed_5g"], dtype=float)
    T_soil = np.asarray(data.heat_grid_data["T_soil"], dtype=float)
    ...
    ax.set_title("5G fixed rail temperatures and soil temperature")
    ax.grid(True)
    ax.legend()
    plt.tight_layout()
    ...
    temperature_plot_path = os.path.join(
        param["dir_result"],
        f"temperature_5g_{data.scenario_name}.png"
    )
    ...
    total_pipe_length = float(
        sum(pipe["length"] for pipe in data.pipeline.values())
    )

    total_useful_heat_demand_5g = float(
        np.sum(param.get("net_useful_heat_demand_5g", 0.0))
    )

    total_cooling_demand_5g = float(
        np.sum(param.get("net_cooling_demand_5g", 0.0))
    )

    total_heat_extraction_5g = float(
        np.sum(param.get("net_heat_extraction_5g", 0.0))
    )

    total_heat_rejection_5g = float(
        np.sum(param.get("net_heat_rejection_5g", 0.0))
    )

    total_thermal_balance_5g = float(
        np.sum(param.get("net_thermal_balance_5g", 0.0))
    )

    total_decentral_HP_el_5g = float(
    ...
    results = {
        "model": {
            "value": "5g_fixed",
            "unit": "-",
            "description": "5G fixed-temperature warm/cold rail network model"
        },
        "pipe_class": {
        ...
        "T_warm_mean_5g": {
            ...
            "unit": "°C",
            "description": "Annual average fixed warm rail temperature"
        },
        "T_cold_mean_5g": {
            "value": float(np.mean(T_cold)),
            "unit": "°C",
            "description": "Annual average fixed cold rail temperature"
        },
        "deltaT_5g_mean": {
            "value": float(np.mean(T_warm - T_cold)),
            "unit": "K",
            "description": "Annual average warm/cold rail temperature difference"
        },
        "total_pipe_length": {
        ...
        "total_cooling_demand_5g": {
            ...
            "unit": "kWh",
            "description": "Useful cooling demand covered by direct cooling"
        },
        "total_heat_extraction_from_network_5g": {
            "value": total_heat_extraction_5g,
            "unit": "kWh",
            "description": "Heat extracted from the warm rail by decentralized heat pumps"
        },
        "total_heat_rejection_to_network_5g": {
            "value": total_heat_rejection_5g,
            "unit": "kWh",
            "description": "Heat rejected to the warm rail by direct cooling"
        },
        ...
        "total_decentral_HP_electricity_5g": {
            ...
            "unit": "kWh",
            "description": "Electricity demand of decentralized heat pumps; not included in network costs here"
        },
        ...
        "substation_ann_costs": {
            ...
            "unit": "€",
            "description": "Annualized substation investment"
        },
        "substation_om_costs": {
            "value": float(substation_om_costs),
            "unit": "€",
            "description": "Substation O&M costs"
        },
        "pipes_ann_costs": {
...
def prepare_result_folder(data, param):
    ...
    result_folder = f"{data.scenario_name}_{topology}_5g_fixed"
...
def plot_random_pipe_massflow_layouts_5g(
        data,
        param,
        n_times=5,
        seed=42,
    ...
    """
    Plot signed 5G pipe massflows on the network layout
    for random timesteps.
    ...
    The plots use:
        - line color for sign and magnitude
        - line width for absolute magnitude
        - midpoint labels for massflow values in kg/s
    """
    ...
    if len(data.pipeline) == 0:
        raise ValueError(
            "data.pipeline is empty. Run initialize_pipeline_from_topology "
            "and calc_flow_5g_fixed first."
        )

    rho_f = float(data.heat_grid_data["fluid"]["rho_f"])

    # ------------------------------------------------------------------
    # Read massflow profiles
    # ------------------------------------------------------------------
    massflow_by_pipe = {}

    for pipe_id, pipe in data.pipeline.items():

        if "massflow_5g" in pipe:
            m = np.asarray(pipe["massflow_5g"], dtype=float)

        elif "flow" in pipe:
            m = np.asarray(pipe["flow"], dtype=float) * rho_f

        else:
            raise KeyError(
                f"Pipe {pipe_id} has neither 'massflow_5g' nor 'flow'. "
                "Run calc_flow_5g_fixed first."
            )

        massflow_by_pipe[pipe_id] = m

    T_len = len(next(iter(massflow_by_pipe.values())))
    ...
    if only_active_timesteps:
        total_abs_massflow = np.zeros(T_len, dtype=float)

        for m in massflow_by_pipe.values():
            total_abs_massflow += np.abs(m)

        candidate_timesteps = np.where(total_abs_massflow > 1e-9)[0]

        if len(candidate_timesteps) == 0:
            raise ValueError(
                "No active massflow timesteps found. "
                "All pipe massflows are zero."
            )

    else:
        candidate_timesteps = np.arange(T_len)

    n_select = min(n_times, len(candidate_timesteps))

    selected_timesteps = np.sort(
        rng.choice(
            candidate_timesteps,
            size=n_select,
            replace=False
    ...
    selected_timesteps = np.asarray(
        [5030, 5054, 5006, 5270, 5174],
        dtype=int
    )

    # ------------------------------------------------------------------
    # Global color scale over selected timesteps
    # ------------------------------------------------------------------
    selected_values = []

    for pipe_id in data.pipeline.keys():
        m = massflow_by_pipe[pipe_id]
        selected_values.extend(m[selected_timesteps])

    max_abs_m = max(float(np.max(np.abs(selected_values))), 1e-12)
    ...
    def _line_width_abs(value_abs):
        if max_abs_m <= 1e-12:
            return 2.0
        return 1.0 + 5.0 * value_abs / max_abs_m
    ...
    # ------------------------------------------------------------------
    # Create one network-layout plot per selected timestep
    # ------------------------------------------------------------------
    for t in selected_timesteps:
        ...
        for pipe_id, pipe in data.pipeline.items():
            ...
            m = float(massflow_by_pipe[pipe_id][t])

            color = cmap(norm(m))
            linewidth = _line_width_abs(abs(m))
        ...
        # Optional: plot nodes
        for node_id, node in data.pipeline_nodes.items():
            pos = tuple(node["pos"])
        ...
        cbar.set_label("Signed mass flow [kg/s]")

        title = (
            f"5G signed pipe massflows at timestep {int(t)}\n"
            "positive = warm → building → cold, "
            "negative = cold → building → warm"
        )

        filename = (
            f"pipeline_massflow_5g_timestep_{int(t)}_"
            f"{data.scenario_name}"
        )
    ...
    print(
        "5G pipe massflow layout plots saved to:",
        dir_result
    )
    print(
        "Selected timesteps:",
        selected_timesteps.tolist()
    )
