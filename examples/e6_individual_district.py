...
def example6_individual_district():
    ...
    ### In this example we want to change some specific assumptions about the district.
    ### Therefore we have to change some default data in the json-files within the "districtgenerator/data/" folder.

    # 1. Changing the location of the district:
    # We have to adjust data in "data/site_data.json". Open the file (e.g. with a text editor). We change
    ...
    # Which climate zone belongs to which city is listed in design_weather_data.json
    # in the folder 'districtgenerator/data\'.

    # 2. Change the weather data base
    # Next, we change the database of the weather data. To do this, we change "districtgenerator/data/site_data.json".
    ...
    # in the folder 'districtgenerator/data' and change "T_set_min" to 18,0°C.

    # 4. Changing the time resolution:
    # To change the time resolution of the profiles open the json-file with the name "time_data"
    # in the directory "districtgenerator/data/" of the districtgenerator. Change the value of "timeResolution" to
