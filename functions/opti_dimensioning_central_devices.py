...
def run_optim(devs, param, dem, result_dict):
    ...
    for device in ["ELYZ", "FC", "SAB", "import"]:
        ...
        for d in days:
            hydrogen[device][d] = {}
            for t in time_steps:
                hydrogen[device][d][t] = model.addVar(vtype="C",
                                                      name="hydrogen_" + device + "_d" + str(d) + "_t" + str(t))
    ...
    for device in ["TES", "CTES", "BAT", "H2S", "GS"]:
        ...
        for d in days:
            ...
            for t in time_steps:
                ...
                ch[device][d][t] = model.addVar(vtype="C", lb=-gp.GRB.INFINITY,
    ...
    for device in ["WT", "WAT", "HP", "EB", "CC", "AC", "CHP", "BOI", "GHP", "BCHP", "BBOI", "WCHP", "WBOI", "ELYZ",
                   "FC", "H2S", "SAB", "TES", "CTES", "BAT", "GS"]:  # PV/STC is not listed due to min_area/max_area.
    ...
    for d in days:
        for t in time_steps:
            # Heating balance
            model.addConstr(
                heat["STC"][d][t] + heat["HP"][d][t] + heat["EB"][d][t] + heat["CHP"][d][t] + heat["BOI"][d][t] +
                heat["GHP"][d][t] + heat["BCHP"][d][t] + heat["BBOI"][d][t] + heat["WCHP"][d][t] + heat["WBOI"][d][t] +
                heat["FC"][d][t] == dem["heat"][d][t] + heat["AC"][d][t] + ch["TES"][d][t])

            # Electricity balance
            model.addConstr(
                power["PV"][d][t] + power["WT"][d][t] + power["WAT"][d][t] + power["CHP"][d][t] + power["BCHP"][d][t] +
                power["WCHP"][d][t] + power["FC"][d][t] + power["from_grid"][d][t] == dem["power"][d][t] +
                power["HP"][d][t] + power["EB"][d][t] + power["CC"][d][t] + power["ELYZ"][d][t] + ch["BAT"][d][t] +
                power["to_grid"][d][t])
            ...
            # Gas balance
            model.addConstr(
                gas["from_grid"][d][t] + gas["SAB"][d][t] == gas["CHP"][d][t] + gas["BOI"][d][t] + gas["GHP"][d][t] +
                ch["GS"][d][t] + gas["to_grid"][d][t])

            # Hydrogen balance
            model.addConstr(
                hydrogen["ELYZ"][d][t] + hydrogen["import"][d][t] == dem["hydrogen"][d][t] + hydrogen["FC"][d][t] +
                hydrogen["SAB"][d][t] + ch["H2S"][d][t])
    ...
    if param["peak_dem_met_conv"] == False:
        ...
        # Power
        model.addConstr(
            cap["PV"] + cap["WT"] + cap["WAT"] + cap["CHP"] + cap["BCHP"] + cap["WCHP"] + cap["FC"] + grid_limit_el >=
            param["peak_power"])
        ...
        for day in year:
            for t in np.arange(1, len(time_steps)):
                ...
                model.addConstr(soc[device][day][t] == soc[device][day][t - 1] * (1 - devs[device]["sto_loss"]) +
                                ch[device][sigma[day]][t])

            # Transition between two consecutive days
            if day > 0:
                model.addConstr(
                    soc[device][day][0] == soc[device][day - 1][len(time_steps) - 1] * (1 - devs[device]["sto_loss"]) +
                    ch[device][sigma[day]][0])

        # Cyclic year condition
        model.addConstr(
            soc[device][0][0] == soc[device][len(year) - 1][len(time_steps) - 1] * (1 - devs[device]["sto_loss"]) +
            ch[device][sigma[0]][0])
    ...
    model.addConstr(from_gas_grid_total == sum(
        sum(gas["from_grid"][d][t] for t in time_steps) * param["day_weights"][d] for d in days))
    model.addConstr(
        to_gas_grid_total == sum(sum(gas["to_grid"][d][t] for t in time_steps) * param["day_weights"][d] for d in days))

    # Total electric energy from and to grid
    model.addConstr(from_el_grid_total == sum(
        sum(power["from_grid"][d][t] for t in time_steps) * param["day_weights"][d] for d in days))
    model.addConstr(to_el_grid_total == sum(
        sum(power["to_grid"][d][t] for t in time_steps) * param["day_weights"][d] for d in days))

    # Total amount of biomass imported
    model.addConstr(
        biom_import_total == sum(sum(biom["import"][d][t] for t in time_steps) * param["day_weights"][d] for d in days))

    # Total amount of waste imported
    model.addConstr(waste_import_total == sum(
        sum(waste["import"][d][t] for t in time_steps) * param["day_weights"][d] for d in days))

    # Total amount of hydrogen imported
    model.addConstr(hydrogen_import_total == sum(
        sum(hydrogen["import"][d][t] for t in time_steps) * param["day_weights"][d] for d in days))
    ...
    model.addConstr(obj["tac"] == sum(c_total[dev] for dev in all_devs)  # annualized investments
                    ...
                    + (from_gas_grid_total * param["co2_gas"] + biom_import_total * param[
        "co2_biom"] + waste_import_total * param["co2_waste"]) * param["co2_tax"])  # CO2 tax
    ...
    if model.Status in (3, 4) or model.SolCount == 0:  # "INFEASIBLE" or "INF_OR_UNBD"
        ...
        for k in ["TES", "CTES"]:
            result_dict[k]["vol_liter"] = round(cap[k].X / (param["c_w"] * param["rho_w"] * devs[k]["delta_T"]) * 3600,
                                                1)
        #
        #        # Calculate emissions
